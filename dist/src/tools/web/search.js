...
export function createWebSearchTool() {
    return {
        name: "web_search",
        label: "Web Search",
        description: "Search the web (DuckDuckGo HTML backend, no API key). Returns the top results as { title, url, snippet } JSON. Use it to research topics, verify facts, or explore anything outside Telegram.",
        parameters: WebSearchSchema,
        execute: async (_toolCallId, params) => {
            const maxResults = Math.min(Math.max(1, params.maxResults ?? 8), 20);
