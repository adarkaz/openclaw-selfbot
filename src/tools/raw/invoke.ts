...
const InvokeSchema = Type.Object(
  {
    method: Type.String({
      description:
        "MTProto API method name, e.g. 'messages.SendMessage', 'messages.GetHistory', 'messages.GetDialogs', 'messages.GetMessages', 'messages.ReadHistory', 'contacts.ResolveUsername', 'contacts.GetContacts', 'channels.GetChannels', 'users.GetUsers', 'users.GetFullUser', 'account.UpdateProfile', etc.",
    }),
    params: Type.Object(
      {},
      {
        ...
        description:
          "Method parameters as key-value JSON. Nested TL objects use a snake_case '_' marker, e.g. {_: 'inputPeerSelf'} or {_: 'inputReplyToMessage', replyToMsgId: 5}. " +
